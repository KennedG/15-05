...
module.exports = {
    ...
    options: [
      {
        name:'user',
        description:'Usuário que desejar abraçar!',
        type:"USER",
        required:true,
      },
    ],
  ...
  if (!pessoa) return interaction.reply({ content: `❌ | ${interaction.member.user} Mencione alguém para abraçar!`});

    if (pessoa.id == interaction.member.user.id) return interaction.reply({ content: `❌| ${interaction.member.user} você não pode se abraçar ;-;!`});
...
const filter1 = (interaction) => {
            if(interaction.user.id == pessoa) return true; 
                ...
                interaction.reply({ content: `❌ apenas o ${pessoa}  tem permissão de reagir no botão`, ephemeral: true })
            }
        }
